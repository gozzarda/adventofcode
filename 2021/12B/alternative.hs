...
-- (current location, previously visited set of (small) vertices, what freebie we took)
type State = (Vertex, Set Vertex, Maybe Vertex)

pathCounts :: Vertex -> Vertex -> AdjMap -> Map State Int
pathCounts src dst adj = dpt
  where
    dps = [ (v, s, b) | v <- Map.keys adj, s <- Set.elems $ Set.powerSet $ Set.delete dst $ Map.keysSet adj, b <- Nothing:(map Just $ filter (not . (`elem` [src, dst, v])) $ Map.keys adj) ]
    dpt = Map.mapWithKey (\s _ -> dpf s) $ Map.fromList $ zip dps $ repeat ()
    dpf (v, s, b) | v == src = if Set.null s && isNothing b then 1 else 0
    dpf (v, s, b) = sum $ map rf ps
      where
        es = Map.findWithDefault Map.empty v adj
        ps = Set.elems $ Set.intersection s $ Map.keysSet es
        rf p = (Map.findWithDefault 0 (if b == Just p then (p, s, Nothing) else (p, Set.delete p s, b)) dpt) * (Map.findWithDefault 0 p es)

numPaths :: AdjMap -> Int
numPaths = sum . Map.elems . Map.filterWithKey (\(v, s, _) _ -> v == "end" && Set.member "start" s) . pathCounts "start" "end"
