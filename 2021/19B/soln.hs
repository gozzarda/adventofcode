...
-- (Scanners, Beacons)
type Points = (Set V3I, Set V3I)

both :: (a -> b) -> (a, a) -> (b, b)
both f (x, y) = (f x, f y)

both2 :: (a -> b -> c) -> (a, a) -> (b, b) -> (c, c)
both2 f (lx, ly) (rx, ry) = (f lx rx, f ly ry)
...
maybeOffset ls rs = listToMaybe $ filter ff $ map mf ds
  where
    ds = [ vecSub l r | l <- Set.elems $ snd ls, r <- Set.elems $ snd rs ]
    mf d = both (Set.map (vecAdd d)) rs
    ff s = (Set.size $ Set.intersection (snd ls) $ snd s) >= 12

-- The second set transformed to match the first if possible
maybeTransformed :: Set M3I -> Points -> Points -> Maybe Points
maybeTransformed rots ls rs = firstJust $ map (maybeOffset ls) $ map (\m -> both (Set.map (matVecMul m)) rs) $ Set.elems rots
...
initState (v:vs) = State { fixed = (Set.empty, Set.empty), todo = [v], unseen = vs }

stepState :: Set M3I -> State -> State
stepState rots state = State { fixed = fixed', todo = todo', unseen = unseen' }
  where
    (u:ts) = todo state
    fixed' = both2 Set.union u $ fixed state
...
statState :: State -> Int
statState = maximum . manhattanDists . Set.elems . fst . fixed

manhattanDists :: [V3I] -> [Int]
manhattanDists ps = [ f l r | (l:rs) <- tails ps, r <- rs ]
  where
    f l r = (\(V3 x y z) -> abs x + abs y + abs z) $ vecSub l r

solve :: Case -> Soln
solve bss = statState $ head $ dropWhile (not . haltState) $ iterate step $ (initState ps)
  where
    ps = zip (repeat $ Set.singleton $ V3 0 0 0) $ map Set.fromList bss
    step = stepState allRotations
