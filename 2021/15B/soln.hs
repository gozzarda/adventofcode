...
tileIncModCols :: Int -> [[Int]] -> [[Int]]
tileIncModCols n = map (concat . take n . iterate (map incMod))

tileIncModRows :: Int -> [[Int]] -> [[Int]]
tileIncModRows n = concat . take n . iterate (map $ map incMod)

tileIncModGrid :: Int -> Int -> [[Int]] -> [[Int]]
tileIncModGrid r c = tileIncModRows r . tileIncModCols c

solve :: Case -> Soln
solve grid = snd $ Map.findMax $ statState $ last states
  where
    wm = gridToMap $ tileIncModGrid 5 5 $ grid
    step = stepState wm
