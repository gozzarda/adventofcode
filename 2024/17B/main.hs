...
solve :: Prob -> Soln
solve (regs, prog) = fromJust $ gos [0 .. 7]
  where
    rom = listArray (0, fromIntegral $ length prog - 1) prog
    (ra, rb, rc) = regs
    gos = listToMaybe . mapMaybe go
    go n = case fromMaybe [] $ execute rom (n, rb, rc) of
      out | out == prog -> Just n
      out | n > 0 && out `isSuffixOf` prog -> gos $ map (+ 8 * n) [0 .. 7]
      _ -> Nothing
...
execute :: ROM -> Regs -> Maybe [Word]
execute rom regs = fmap statState haltState
  where
