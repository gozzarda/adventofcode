...
-- Limits allowed in straight line
...
neighbours iwm (pos, dir@(dr, dc), steps) = catMaybes $ map (\v -> fmap ((,) v) $ weight v) $ catMaybes $ map next dirs
  where
    ...
    next d = case d of
      d | dir == (0, 0) -> Just (addCoord pos d, d, 1)
      d | d == dir -> if steps < maxSteps then Just (addCoord pos d, d, steps + 1) else Nothing
      (dr', dc') | dr' == (-dr) && dc' == (-dc) -> Nothing
      d -> if steps >= minSteps then Just (addCoord pos d, d, 1) else Nothing
