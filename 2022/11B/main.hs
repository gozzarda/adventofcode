...
makeRule :: Worry -> Oper -> Test -> Rule
makeRule ring oper test old = let new = ((`mod` ring) . makeOper oper) old in (makeTest test new, new)
...
makeRules ms = IntMap.fromList $ map (\(Monkey n _ o t) -> (n, makeRule ring o t)) ms
  where
    ring = product $ map (\(Monkey _ _ _ (Test d _)) -> d) ms
...
doGame rm s = iterate (doRound rm) s !! 10000
