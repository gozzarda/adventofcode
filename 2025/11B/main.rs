...
fn main() {
    ...
    let svr = tla_to_id("svr").unwrap();
    let dac = tla_to_id("dac").unwrap();
    let fft = tla_to_id("fft").unwrap();
    let out = tla_to_id("out").unwrap();
    let sdfo = num_paths(&adj, svr, dac) * num_paths(&adj, dac, fft) * num_paths(&adj, fft, out);
    let sfdo = num_paths(&adj, svr, fft) * num_paths(&adj, fft, dac) * num_paths(&adj, dac, out);
    let result = sdfo + sfdo;
